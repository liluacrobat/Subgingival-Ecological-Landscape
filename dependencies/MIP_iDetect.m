function [Objective, Weight, History] = MIP_iDetect(patterns, Para)
...
%       Para.soft:  if 0, using k nearest neighbour to determine the kernel
%                   width
%   Para.distance:  distance metric. 'euclidean' or 'block' (default)
%      Para.sigma:  kernel width; represent k nearest neighbour if
%                   Para.soft=0
%     Para.lambda:  regularization parameter
%                   (can be tuned by gap statistics, see reference)
%     Para.kernel:  kernel function
%
%
%
%OUTPUT:
...
%% initilization
if isfield(Para,'start')
    Weight=Para.start/norm(Para.start);
else
...
while Difference>0.001 && t<=T
    ...
    for i = 1:N_patterns
        ...
        if Para.soft==1
            prob = exp(-(dist-dist(temp_index(2)))/sigma);prob(i) = 0;
        else
            % Assign the probability of k nearest neighbors
            prob=kernel_fun(dist,Para,i);
        end
    ...
    C = sum(FN-alpha*NN,2);C_original = C;
    C(C<0) = 0;
    Weight = C/norm(C);
...
function prob = kernel_fun(dist,Para,iid)
[~,I] = sort(dist);
prob = zeros(size(I));
kernel = Para.kernel;
dis_neighbor = dist(I(1:Para.sigma+2));
dis_neighbor = dis_neighbor/max(dis_neighbor);
switch kernel
    case 'triangular'
        prob(I(1:Para.sigma+2)) = 1-abs(dis_neighbor);
    case 'parabolic'
        prob(I(1:Para.sigma+2)) = (1-(dis_neighbor).^2)*3/4;
    case 'quartic'
        prob(I(1:Para.sigma+2)) = (1-(dis_neighbor).^2).^2;
    case 'triweight'
        prob(I(1:Para.sigma+2)) = (1-(dis_neighbor).^2).^3;
    case 'tricube'
        prob(I(1:Para.sigma+2)) = (1-(abs(dis_neighbor)).^3).^3;
    case 'cosine'
        prob(I(1:Para.sigma+2)) = cos(pi/2*dis_neighbor);
    otherwise
        prob(I(1:Para.sigma+2)) = 1;
end
prob(iid) = 0;
prob = prob/sum(prob);
%% ==================End of the code===================================
